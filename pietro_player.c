...
unsigned char player_move(void){
	...
	if ( BIT_CHK(s_state, STAT_KILL) ) {
		if ( game_check_time(spr_timer[sprite],50) ) {
	...
	if ( BIT_CHK( state_a[sprite], STAT_PUSH) ) {
		if ( game_check_time(spr_timer[sprite], 20 ) ) {
			//CLEAR PUSH
