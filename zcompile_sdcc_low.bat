...
del pietro_release_sdcc_low.tap > nul 2>&1
...
zcc +zx -vn -zorg=%PADDR% -startup=31 -SO3 -clib=sdcc_iy --max-allocs-per-node3000 --opt-code-size --fsigned-char @zproject.lst -o pietro_bros -pragma-include:zpragma.inc
...
copy /b /Y loader.tap + mcload.tap + pietro_scr.tap + nirvanap.tap + pietro.tap pietro_release_sdcc_low.tap
