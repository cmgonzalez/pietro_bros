...
del bin\pietro_release_llvm.tap > nul 2>&1
...
zcc +zx -vn -zorg=%PADDR% -startup=31 -SO3 -clib=llvm_iy --max-allocs-per-node200000 --opt-code-size --fsigned-char @zproject.lst -o pietro_bros -pragma-include:zpragma.inc
...
copy /b /Y loader.tap + mcload.tap + mcloader.tap + pietro_scr.tap + nirvanap.tap + pietro.tap + pietro_ay.tap bin\pietro_release_llvm.tap
