...
void enemy_turn(void) {
  for (sprite = 0; sprite < 6; ++sprite) {
    if (class[sprite] > 0 && spr_chktime(&sprite)) {
      ...
      // FLIPPED ENEMY
      if (BIT_CHK(s_state, STAT_HIT) && !BIT_CHK(s_state, STAT_FALL) &&
          !BIT_CHK(s_state, STAT_JUMP)) {
...
unsigned char enemy_collision(void) {
  ...
  for (enemies = 0; enemies < SPR_P2; ++enemies) {
    if (class[enemies] == 0 || class[enemies] > FIGHTERFLY)
      continue;
    if (enemies == sprite)
      continue;

    if (BIT_CHK(state[enemies], STAT_KILL))
      continue;
    if (BIT_CHK(state[enemies], STAT_FALL))
      continue;
...
void enemy_trip_move(unsigned char f_slin, unsigned char f_scol,
                     unsigned char f_dir, unsigned char f_dir_alt) {
  tmp = 1;
  NIRVANAP_halt();
...
void enemy_upgrade(unsigned char f_enemy, unsigned char f_class,
...
void enemy_walk(void) {
  ...
  if (tmp) {
    ...
    tmp = !tmp0 && !tmp1;
    if (tmp && (lin[sprite] - jump_lin[sprite] <= 8)) {
			spr_move_horizontal();
		}
...
void enemy_kill(unsigned char f_sprite) __z88dk_fastcall {
  ...
  if (class[f_sprite] == FIGHTERFLY)
    class[f_sprite] = SHELLCREEPER_GREEN; /* HACK */
  ...
  if (class[f_sprite] <= SIDESTEPPER_MAGENTA && game_type != GAME_RANDOM_TYPE) {
    ...
    if (phase_left == 1) {
      ...
      for (tmp0 = 0; tmp0 < SPR_P2; ++tmp0) {
        if (class[tmp0] > 0 && class[tmp0] <= FIGHTERFLY &&
            !BIT_CHK(state[tmp0], STAT_KILL)) {
          /*evolve twice*/
          enemy_evolve(tmp0);
          enemy_evolve(tmp0);
