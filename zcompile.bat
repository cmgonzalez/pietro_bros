...
del pietro_release.tap > nul 2>&1
...
@rem zcc +zx -vn -zorg=%PADDR% -startup=31 -SO3 -clib=sdcc_iy --max-allocs-per-node200000 --opt-code-size --fsigned-char @zproject.lst -o pietro_bros -pragma-include:zpragma.inc
zcc +zx -vn -zorg=%PADDR% -startup=31 -O3 -clib=new @zproject.lst -o pietro_bros -pragma-include:zpragma.inc
...
copy /b /Y loader.tap + mcload.tap + pietro_scr.tap + nirvanap.tap + pietro.tap pietro_release.tap

del loader.tap mcload.tap nirvanap.tap nirvanap.bin nirvanap_final.bin > nul 2>&1
...
del pietro_scr.bin pietro_scr.tap > nul 2>&1
