...
unsigned char spr_check_over(){

	if (s_lin0 >= 136) {
		if( s_col0 < 4) {
			if (game_ugly_back != 3) spr_back_paint(0 + 15 * 32);
			game_ugly_back = 3;
			return 1;
		}
		if( s_col0 > 26) {
			if (game_ugly_back != 4) spr_back_paint(26 + 15 * 32);
			game_ugly_back = 4;
			return 1;
		}
	} else {
		if (s_lin0 <= 40) {
			if( s_col0 < 4) {
				if (game_ugly_back != 1) spr_back_paint(0);
				game_ugly_back = 1;
				return 1;
			}
			if( s_col0 > 26) {
				if (game_ugly_back != 2) spr_back_paint(26);
				game_ugly_back = 2;
				return 1;
			}
		}
...
void spr_back_fix( unsigned char f_inc ) __z88dk_fastcall {
	  unsigned char t1;
		...
		if (v1 || v2 ) {
			...
			if (t1 == TILE_BRICK_FREEZE || t2 == TILE_BRICK_FREEZE) {
					NIRVANAP_drawT( TILE_BRICK_FREEZE , s_lin1, s_col1 );
			} else {
